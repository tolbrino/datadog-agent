...
func checkRights(filename string) error {
	...
	for i := uint32(0); i < aclSizeInfo.AceCount; i++ {
		...
		if pAce.AceType == winutil.ACCESS_DENIED_ACE_TYPE {
			...
			if compareIsLocalSystem || compareIsAdministrators || compareIsSecretUser {
				return fmt.Errorf("Can't deny access to LOCAL_SYSTEM or Administrators or secret user on %s", filename)
			}
			// otherwise, it's fine; deny access to whomever
		}
		if pAce.AceType == winutil.ACCESS_ALLOWED_ACE_TYPE {
			if !(compareIsLocalSystem || compareIsAdministrators || compareIsSecretUser) {
				return fmt.Errorf("Can't allow access to users/groups other than LOCAL_SYSTEM or Administrators or secret user on secretBackendCommand '%s'", filename)
			}
	...
	if !bSecretUserExplicitlyAllowed {
		...
		return fmt.Errorf("Secret user not allowed to execute secretBackendCommand '%s'", filename)
	}
	return nil
}
